...
from ETL.DataHandler import (
    download_dataset,
    read_text_file_and_process_df,
    convert_data_set_xy,
    get_n_fold,
)
from Utils.CommonUtils import load_json, write_json

if __name__ == "__main__":

    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--experiment_config", help="experiment config.json", default="exp_config.json"
    )
    ...
    if not os.path.exists(expanded_csv_path) and not os.path.exists(
        word_file_path
    ):  # if processed files are not present
        print(
            " Open Jupyter notebook and run Notebooks/Data_visualization.ipynb to get the processed and fixed data"
        )
        sys.exit(0)
    else:
        data_df = pd.read_csv(expanded_csv_path, index_col=0)
        word_df = pd.read_csv(word_file_path, index_col=0)
        dataset = convert_data_set_xy(df_dataset=data_df, df_word_list=word_df)
        ...
        num_of_fold = exp_config.get("data", None).get("num_of_fold", 5)
        data_dict = get_n_fold(num_of_fold=num_of_fold, dataset=dataset)
        write_json(
            output_path=os.path.join(
                exp_config.get("data", "data").get("dataset_dir", None),
                exp_config.get("data").get("cv_split_file", None),
            ),
            data=data_dict,
        )
